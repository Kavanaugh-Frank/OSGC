...
import pandas as pd
from main_function.ecef import (
    latlon_to_ecef,
)  # Ensure this import is correct and the function exists


def interpolation(
    df, num_x_slice, num_y_slice, lat, long, lat_difference, long_difference, point
):
    """
    ...
    if x_resolution < 2 or y_resolution < 2:
        raise ValueError(
            "DataFrame must have at least two rows and columns for interpolation."
        )
    ...
    for i in range(num_x_slice):
        index = min(
            math.floor((i * x_percent_spacing / 100) * (x_resolution - 1)),
            x_resolution - 1,
        )
        x_indices.append(index)
        x_indices_decimal.append(
            ((i * x_percent_spacing) / 100) * (x_resolution - 1) % 1
        )

    y_indices, y_indices_decimal = [], []
    for i in range(num_y_slice):
        index = min(
            math.floor((i * y_percent_spacing / 100) * (y_resolution - 1)),
            y_resolution - 1,
        )
        y_indices.append(index)
        y_indices_decimal.append(
            ((i * y_percent_spacing) / 100) * (y_resolution - 1) % 1
        )
    ...
    for x_counter in range(num_x_slice):
        ...
        for y_counter in range(num_y_slice):
            ...
            bottom_right_value = df.iat[
                min(x_index + 1, x_resolution - 1), min(y_index + 1, y_resolution - 1)
            ]
            ...
            X, Y, Z = (
                ...
                Decimal(Z) - Decimal(point_ecef[2]),
            )
