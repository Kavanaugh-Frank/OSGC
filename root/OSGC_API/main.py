...
from main_function.shrink_array import interpolation # Does the interpolation of the data into the smaller datasets
...
def process_coordinates():
    ...
    if not (num_x_slice > 0 and num_y_slice > 0):
    ...
    try:
        ...
        abort(404, f"Interpolation failed with error: {e}. Parameters: num_x_slice={num_x_slice}, num_y_slice={num_y_slice}, upper_lat={upper_lat}, upper_long={upper_long}, lower_lat={lower_lat}, lower_long={lower_long}, gs_lat={gs_lat}, gs_long={gs_long}, gs_height={gs_height}")
    try:
    ...
    # this needs to be done this way because each data point is a 3 item numpy list
    # and needs to be converted back to pure python for Flask
    flattened_data = []
